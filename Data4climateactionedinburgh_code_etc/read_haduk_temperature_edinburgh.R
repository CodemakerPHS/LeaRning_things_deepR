...
# CONFIG ####
data_dir <- here("data", "temperature")
...
#### Example: extract daily tasmax and tasmin for 2024 etc then produce plot ####
...
# FIX: Ensure all columns are atomic (not list-columns) before writing CSV
...
if (nrow(daily_combined) > 0) {
  p1 <- ggplot(daily_combined, aes(x = date, y = value, color = type)) +
    geom_line(alpha = 0.7) +
    labs(
      title = paste0("HadUK temperatures at ", city_name),
      subtitle = paste0(
...
if (nrow(ts_mon) > 0) {
  ...
  p2 <- ggplot(ts_mon, aes(x = date, y = value)) +
    ...
    labs(
      title = paste0("Monthly mean temperature (HadUK) at ", city_name),
      subtitle = paste0(
        "Point nearest HadUK grid cell to (",
        city_lat,
        ", ",
...
if (nrow(daily_combined) > 0) {
  ...
  p3 <- ggplot(
    ...
    geom_line() +
    labs(
      title = paste0("Monthly means computed from daily values at ", city_name),
      x = "Month",
      y = "Temperature (°C)"
    ) +
    theme_minimal()
